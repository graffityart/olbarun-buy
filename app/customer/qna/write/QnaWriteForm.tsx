...
export default function QnaWriteForm(){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      setStatus('문의가 등록되었습니다.'); form.reset(); await loadCaptcha();
    }catch(err){setStatus(err instanceof Error?err.message:'등록 중 오류가 발생했습니다.'); await loadCaptcha();}
  ...
  return <form className={styles.form} onSubmit={submit}>
    ...
    {status&&<p style={{textAlign:'center',fontWeight:800,marginTop:16}}>{status}</p>}
  </form>
}