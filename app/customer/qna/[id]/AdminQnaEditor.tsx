...
export default function AdminQnaEditor({id,title,nickname,content,answer}:{id:string,title:string,nickname:string,content:string,answer:string}){
 ...
 const remove=async()=>{if(!confirm('이 게시글을 삭제하시겠습니까? 삭제 후 복구할 수 없습니다.'))return;setBusy(true);setMsg('');try{const r=await fetch(`/api/admin/qna/${id}`,{method:'DELETE'});const j=await r.json();if(!r.ok)throw new Error(j.message||'삭제 실패');location.href='/customer/qna';}catch(e:any){setMsg(e.message||'삭제 중 오류가 발생했습니다.');setBusy(false);}};
