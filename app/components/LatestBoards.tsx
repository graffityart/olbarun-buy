...
export default async function LatestBoards(){
 ...
 return <section className={styles.section}><div className="container"><div className={styles.grid}>
  ...
  <div className={styles.board}><div className={styles.head}><div><small>PURCHASE REQUEST</small><h2>매입 신청/ 문의하기</h2></div><a href="https://www.pumasy.co.kr/customer/qna" aria-label="1:1 문의 게시판">＋</a></div>{requests.length?<ul className={styles.list}>{requests.map(r=><li className={styles.item} key={String(r.id)}><a className={styles.title} href={`https://www.pumasy.co.kr/customer/qna/${r.id}`}><span>{r.title}</span>{r.status==='completed'&&<b className={styles.answered}>처리완료</b>}</a><time className={styles.date}>{date(r.created_at)}</time></li>)}</ul>:<div className={styles.empty}>등록된 문의가 없습니다.</div>}<div className={styles.boardActions}><a className={styles.more} href="https://www.pumasy.co.kr/customer/qna">1:1 문의 게시판 <span>→</span></a><a className={styles.write} href="https://www.pumasy.co.kr/customer/qna/write">문의하기</a></div></div>
 </div></div></section>;
}
