...
export default async function QnaDetail({params}:Props){
 ...
 const sql=db();
 const rows=await sql`SELECT id,nickname,title,content,is_secret,status,admin_answer,created_at FROM qna_posts WHERE id=${Number(id)} LIMIT 1`;
 const post=rows[0]; if(!post) notFound();
 return <main className={styles.page}>
 <header className="siteHeader"><div className="container navWrap"><a href="/" className="brand"><span className="brandMark">↻</span><span>올바른<span className="brandBlue">매입</span></span></a><nav className="desktopNav"><a href="/#category">폐컴퓨터 매입</a><a href="/#price">매입단가</a><a href="/process">매입절차</a><a href="/request">수거신청서</a><a href="/customer/notice">고객센터</a></nav></div></header>
